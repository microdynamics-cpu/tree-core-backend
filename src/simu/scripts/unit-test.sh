...
CUST_TESTS_BIN_PATH1=/root/Desktop/chisel/tree-core-cpu/rtl/dependency/am/riscv-tests/build/
...
RV64I_CUST_TEST_BIN1=`eval "find $CUST_TESTS_BIN_PATH1 -type f -name \"*-riscv64-mycpu.bin\" ! -name \"*.dump\""`
RV64I_CUST_TEST_BIN2=`eval "find $CUST_TESTS_BIN_PATH2 -type f -name \"*-riscv64-mycpu.bin\" ! -name \"*.dump\""`
...
unitTest() {
    ...
    for file in $RV64I_CUST_TEST_BIN1; do
        val=`eval "basename $file"`
        # echo $val
        printf "$INFO[%16s] $END" $val
